{
  inputs =
    let
      version = "1.10.0";
      system = "x86_64-linux";
      devenv_root = "/home/surgiie/.files";
      devenv_dotfile = "/home/surgiie/.files/.devenv";
      devenv_dotfile_path = ./.devenv;
      devenv_tmpdir = "/run/user/1000";
      devenv_runtime = "/run/user/1000/devenv-42b3f97";
    ...
    // (
      if builtins.pathExists (devenv_dotfile_path + "/flake.json") then
        builtins.fromJSON (builtins.readFile (devenv_dotfile_path + "/flake.json"))
      else
        { }
    );
  ...
  outputs =
    ...
    let
      ...
      devenv =
        if builtins.pathExists (devenv_dotfile_path + "/devenv.json") then
          builtins.fromJSON (builtins.readFile (devenv_dotfile_path + "/devenv.json"))
        else
          { };
        ...
        let
          ...
          baseProject = pkgs.lib.evalModules {
            ...
            ++ [
              ...
              (
                if builtins.pathExists (devenv_dotfile_path + "/cli-options.nix") then
                  import (devenv_dotfile_path + "/cli-options.nix")
                else
                  { }
              )
            ];
