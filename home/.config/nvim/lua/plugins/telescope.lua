-- Add image support to telescope previews: https://github.com/4rd/image.nvim/issues/183
local function telescope_image_preview()
	...
	local function defaulter(f, default_opts)
		...
		return {
			...
			__call = function()
				local okay, err = pcall(f(default_opts))
				if not okay then
...
return {
	...
	config = function()
		...
		local ignored_fts = { ["neo-tree"] = true, ["TelescopePrompt"] = true, ["neo-tree-popup"] = true }
		local function select_avoiding_neotree(prompt_bufnr)
